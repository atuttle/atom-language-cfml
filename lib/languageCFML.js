	...
	activate: function() {
		...
		atom.workspace.emitter.on("did-open", function(tab){
			if (tab.item.getURI().endsWith("cfc")) {
				bindCfcChangeChecker(tab.item);
				return checkCfscriptEditorGrammar(tab.item);
			} else if (tab.item.getURI().endsWith("cfm")) {
				bindCfmlChangeChecker(tab.item);
				return checkCfmlEditorGrammar(tab.item);
			}
		});
		...
		for (var editor of atom.workspace.getTextEditors()) {
			...
			} else if (editor.getURI().endsWith("cfm")) {
				bindCfmlChangeChecker(editor);
				return checkCfmlEditorGrammar(editor);
			}
		}
	}
};
...
function checkCfmlEditorGrammar(editor) {
	var cfmlHasHtmlPattern = new RegExp('<(?!cf)(?:[a-z:]*)', "i");
	if (!editor._checking_is_cfml) {
		editor._checking_is_cfml=true;

		if (!!module.cfmlGrammar && !!module.htmlCfmlGrammar) {
			//	Only run this is the grammars have been defined.
			for (var line of editor.getBuffer().lines) {
				if (line.length) {
					if (cfmlHasHtmlPattern.test(line)) {
		...
		editor._checking_is_cfml=false;
	}
}

function bindCfmlChangeChecker(editor) {
	editor._changeChecker = editor.emitter.on("did-change", function() {
		checkCfmlEditorGrammar(editor);
	});
}
